...
export const addAddress = () => {
  ...
  const queryClient = useQueryClient();
  const { closeModal } = useModalAction();
  return useMutation(client.users.add, {
    onSuccess: (data) => {
      if (data?._id) {
        closeModal();
      }
  ...
  const { mutate, isLoading } = useMutation(client.users.login, {
    onSuccess: async (data) => {
      ...
      if (!data?.user?.isActive) {
        setServerError('You are Blocked');
        return;
      }
      ...
      setToken(data.token);
      const cartData = localStorage.getItem('pick-cart');
      if (cartData) {
        try {
          const items = JSON.parse(JSON.parse(cartData));
          for (const item of items.items) {
            await addItemToCart(item, item.quantity);
          }
        } catch (error) {
    ...
    onError: (error: Error) => {
      ...
      setServerError('error-correct-password-required');
    },
  });
...
export function useSendOtpCode({
  ...
  const { mutate, isLoading } = useMutation(client.users.sendOtpCode, {
    ...
    onError: (error: Error) => {
      toast.error(`${t('erroe-enter-valid-phonenumber')}`);
  ...
  const { mutate: otpLogin, isLoading } = useMutation(client.users.OtpLogin, {
    onSuccess: (data) => {
      if (!data.token) {
        ...
        toast.error(t('text-otp-verify-failed')); // Show toast for incorrect OTP
        return;
      }
      setToken(data.token!);
      setAuthorized(true);
    ...
    onError: (error: AxiosError) => {
      if (error.response?.status === 400 && error.response?.data?.message === 'Failed to verify OTP') {
...
export function useRegister() {
  ...
  const { mutate, isLoading } = useMutation(client.users.register, {
    onSuccess: (data) => {
      if (data?.user) {
        closeModal();
        toast.success(`${t('error-email-verify')}`);
        openModal('VERIFY_EMAIL');
...
export function useResendVerificationEmail() {
  ...
  const { mutate, isLoading } = useMutation(
    (email) => client.users.resendVerificationEmail(email),
    {
      onSuccess: (data) => {
        if (data?.success) {
...
export function useEmailVerification() {
  const router = useRouter();
  const { mutate, isLoading } = useMutation(
    (token) => client.users.emailVerify(token),
    {
      onSuccess: (data) => {
        if (data.success) {
  ...
  const { mutate, isLoading } = useMutation(client.users.changePassword, {
    ...
    onError: (error) => {
      ...
      if (
        ...
        setFormError({
          oldPassword: t('error-password-mismatch'),
        });
      } else if (
        data?.statusCode === 404 &&
        ...
        setFormError({
          oldPassword: t('error-user-not-found'),
        });
      } else {
        setFormError(data);
      }
...
export function useVerifyForgotPasswordToken() {
  ...
  const { mutate, isLoading } = useMutation(
    client.users.verifyForgotPasswordToken,
    {
      onSuccess: (data, variables) => {
        if (!data.success) {
          setFormError({ token: data?.message ?? 'Invalid token' });
          return;
        }
      ...
      onError: (error: AxiosError) => {
        if (error.response?.status === 401) {
          setFormError({ token: 'Invalid or expired token' });
        } else {
          console.error('Error verifying token:', error);
          setFormError({ token: 'Failed to verify token' });
        }
      },
      onSettled: () => {
        queryClient.clear();
