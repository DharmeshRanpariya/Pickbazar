...
export const customerContactAtom = atom(
  (get) => get(checkoutAtom).customerContact,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, customerContact: data });
  }
);
export const guestNameAtom = atom(
  (get) => get(checkoutAtom).customerName,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, customerName: data });
  }
);
...
export const discountAtom = atom((get) => {
  const checkoutState = get(checkoutAtom);
  return checkoutState.coupon?.discountAmount;
});
