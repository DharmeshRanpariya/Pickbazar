...
export function addItemWithQuantity(
  ...
  const existingItemIndex = items.findIndex(
    (existingItem) => existingItem.product._id === item.product._id,
  );
...
export function getItem(items: Item[], id: Item['id']) {
  return items.find((item) => item.product?._id === id);
}
...
export function removeItem(items: Item[], id: Item['id']) {
  return items.filter((existingItem) => existingItem.product?._id !== id);
}
