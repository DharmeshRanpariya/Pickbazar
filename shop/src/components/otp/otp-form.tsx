...
interface OtpFormProps {
  ...
  onVerifySuccess: (values: { phoneNumber: string }) => void;
}
export default function OtpForm({
  ...
  function onSendCodeSubmission({ phoneNumber }: { phoneNumber: string }) {
    sendOtpCode({
  ...
  function onVerifyCodeSubmission({ code }: { code: string }) {
    console.log(otpState, otpState.phoneNumber, 'pjjj');
    verifyOtpCode({
      code,
