      ...
      <ul className="space-y-2 text-base [&>li>p]:font-normal [&>li>p]:text-[#666] [&>li>span]:font-semibold [&>li>span]:text-muted-black [&>li]:flex [&>li]:items-center [&>li]:gap-2">
        {flashSale?.startDate || flashSale?.endDate ? (
          <li>
            ...
            <p>
              {dayjs(flashSale?.startDate).format('DD MMM YYYY')} -{' '}
              {dayjs(flashSale?.endDate).format('DD MMM YYYY')}
            </p>
          </li>
        ) : (
          ''
        ...
        <li>
          ...
          <p>
            {flashSale &&
            new Date() >= new Date(flashSale.startDate) &&
            new Date() <= new Date(flashSale.endDate)
              ? 'On going'
              : 'On hold'}
          </p>
        </li>
