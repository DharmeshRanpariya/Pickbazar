...
const Xenon: React.FC<XenonProps> = ({ product, className }) => {
  ...
  const { price, basePrice, discount } = usePrice({
    amount: product.sale_price
      ? product.sale_price
      : product.price! || variationOptions[0].sale_price,
    baseAmount: product.price || variationOptions[0].price,
  });
  const { openModal } = useModalAction();

  function handleProductQuickView() {
    const formattedName = name.replace(/ /g, '_');
    const url = `products/${formattedName}&ProductId=${_id}`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Image
          src={image[0]?.original ?? productPlaceholder}
          alt={name}
          fill
