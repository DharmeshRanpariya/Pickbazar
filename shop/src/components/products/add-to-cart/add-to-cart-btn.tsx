...
const AddToCartBtn: React.FC<Props> = ({
  ...
  const renderContent = (icon?: JSX.Element, label?: string) => (
    <>
      {loading ? (
      ...
      {label && <span className="ltr:ml-2 rtl:mr-2">{label}</span>}
    </>
  );
  ...
  switch (variant) {
    ...
    case 'argon':
      return (
        ...
        >
          {renderContent(<PlusIcon className="h-4 w-4 stroke-2" />)}
        </button>
      );
    case 'oganesson':
      return (
    ...
    case 'big':
      return (
        ...
        >
          {renderContent(<span>{t('text-add-cart')}</span>)}
        </button>
      );
    case 'text':
      return (
