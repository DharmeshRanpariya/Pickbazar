...
const Details: React.FC<Props> = ({
  ...
  if (isSelected) {
    selectedVariation = product?.variationOptions?.find((o: any) =>
      isEqual(
          ...
          <div className="w-full" ref={intersectionRef}>
            ...
            <div className="mt-6 flex flex-col items-center md:mt-6 lg:flex-row">
              ...
              {!isEmpty(selectedVariation) && (
                <span className="whitespace-nowrap text-base text-body ltr:lg:ml-7 rtl:lg:mr-7 text-red-500">
                  {selectedVariation?.is_disable ||
                    selectedVariation.quantity === 0
                    ? t('text-out-stock')
                    : selectedVariation.quantity < 10
                      ? `Only ${selectedVariation.quantity} ${t(
                        'text-few-pieces-left',
                      )}`
                      : ``}
                </span>
              )}
