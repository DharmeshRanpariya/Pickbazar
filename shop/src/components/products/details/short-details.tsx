...
const ShortDetails: React.FC<ShortDetailsProps> = ({ product, isSticky }) => {
  ...
  return (
    <div
      className={twMerge(
        cn(
          'top-0 left-0 z-50 hidden h-auto w-full max-w-6xl bg-light px-8 py-6 shadow transition-all duration-300  ease-in-out',
          {
    ...
    >
      <div className="flex items-center">
        ...
        >
          <Image
            src={selectedVariation?.image?.original! ?? image[0]?.original}
            alt={name}
            fill
