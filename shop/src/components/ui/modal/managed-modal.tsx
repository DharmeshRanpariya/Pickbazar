...
const EmailVerify = dynamic(() => import('@/components/auth/email-verify'), {
  ssr: false,
});
...
const OrderCancelView = dynamic(
  () => import('@/components/refunds/order-cancel-form'),
);
...
const CouponDetails = dynamic(
  () => import('@/components/ui/cards/coupon-details'),
);
...
const ManagedModal = () => {
  ...
  // }
  return (
    <Modal open={isOpen} onClose={closeModal} disableBackdropClose={data?.disableBackdropClose}>
      {view === 'LOGIN_VIEW' && <Login />}
      {view === 'REGISTER' && <Register />}
      {view === 'VERIFY_EMAIL' && <EmailVerify />}
      {view === 'FORGOT_VIEW' && <ForgotPassword />}
      {view === 'OTP_LOGIN' && <OtpLoginView />}
      {view === 'REFUND_REQUEST' && <CreateRefundView />}
      {view === 'ORDER_CANCEL' && <OrderCancelView />}
      {view === 'ADD_OR_UPDATE_ADDRESS' && (
        <CreateOrUpdateAddressForm data={data} />
      )}
      {view === 'COUPON_DETAILS' && <CouponDetails />}
