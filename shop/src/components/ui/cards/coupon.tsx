...
const CouponCard: React.FC<CouponCardProps> = ({ coupon, className }) => {
  ...
  const handleCouponDetails = () => {
    return openModal('COUPON_DETAILS', coupon);
  };

  return (
    <div className={cn('coupon-card', className)}>
      <div className="relative flex overflow-hidden bg-gray-200 h-[12.5rem] rounded cursor-pointer">
        {image?.[0]?.thumbnail ? (
          <div
            className="blur- absolute top-0 left-0 h-full w-full bg-cover bg-center bg-no-repeat blur-sm"
            style={{ backgroundImage: `url(${image?.[0]?.thumbnail})` }}
          ></div>
        ) : (
          ''
        )}
        <Image
          src={image?.[0]?.thumbnail ?? couponPlaceholder}
          alt={code}
      ...
      <div className="grid items-center w-11/12 grid-flow-col px-5 py-4 mx-auto rounded-bl shadow-sm rounded-be auto-cols-fr bg-light">
        <>
          <span className="flex items-center font-semibold text-heading focus:outline-none gap-1.5">
            {copyText.value}{' '}
