...
export const productClient = {
  ...
  get({ slug, language }: GetParams) {
    return HttpClient2.get<Product>(`${API_ENDPOINTS.PRODUCTS}/${slug}`, {
      language,
  ...
  paginated: ({
    ...
    return HttpClient2.get<ProductPaginator>(API_ENDPOINTS.PRODUCTS, {
      ...params,
      type,
      name,
      categories,
  ...
  lowStock({ shop_id, ...params }: Partial<ProductQueryOptions>) {
    return HttpClient2.get<Product[]>(
  ...
  newOrInActiveProducts: ({
    ...
    return HttpClient2.get<ProductPaginator>(
