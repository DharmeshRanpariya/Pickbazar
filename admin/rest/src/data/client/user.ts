...
export const userClient = {
  me: () => {
    return HttpClient2.get<User>(API_ENDPOINTS.ME);
  },
  login: (variables: LoginInput) => {
    return HttpClient2.post<AuthResponse>(API_ENDPOINTS.LOGIN, variables);
  },
  logout: () => {
  ...
  register: (variables: RegisterInput) => {
    return HttpClient2.post<AuthResponse>(API_ENDPOINTS.REGISTER, variables);
  },
  ...
  changePassword: (variables: ChangePasswordInput) => {
    return HttpClient2.post<any>(API_ENDPOINTS.CHANGE_PASSWORD, variables);
  },
  forgetPassword: (variables: ForgetPasswordInput) => {
    return HttpClient2.post<any>(API_ENDPOINTS.FORGET_PASSWORD, variables);
  },
  verifyForgetPasswordToken: (variables: VerifyForgetPasswordTokenInput) => {
    return HttpClient2.post<any>(
      API_ENDPOINTS.VERIFY_FORGET_PASSWORD_TOKEN,
      variables,
    );
  },
  resetPassword: (variables: ResetPasswordInput) => {
    return HttpClient2.post<any>(API_ENDPOINTS.RESET_PASSWORD, variables);
  },
  ...
  block: (variables: BlockUserInput) => {
    return HttpClient2.post<any>(API_ENDPOINTS.BLOCK_USER, variables);
  },
  unblock: (variables: BlockUserInput) => {
    return HttpClient2.post<any>(API_ENDPOINTS.UNBLOCK_USER, variables);
  },
  ...
  fetchUsers: ({ name, ...params }: Partial<UserQueryOptions>) => {
    return HttpClient2.get<UserPaginator>(API_ENDPOINTS.USERS, {
      searchJoin: 'and',
      with: 'wallet',
      ...params,
      name,
