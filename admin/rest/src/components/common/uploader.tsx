...
export default function Uploader({
        ...
        upload(
          acceptedFiles, // it will be an array of uploaded attachments
          {
            onSuccess: (data: any) => {
              ...
              dataArray.map((file: any, idx: any) => {
                const splitArray = file?.photoUrl?.original?.split('/');
                let fileSplitName =
                ...
                dataArray[idx]['file_name'] = filename + '.' + fileType;
              });

              let mergedData;
              if (multiple) {
                mergedData = files.concat(dataArray);
                setFiles(files.concat(dataArray));
              } else {
                mergedData = dataArray[0];
                setFiles([dataArray[0]]);
              }
              if (onChange) {
                onChange(mergedData);
              }
  ...
  const handleDelete = (image: string) => {
    const images = files.filter((file) => file?.photoUrl?.thumbnail !== image);
    setFiles(images);
    if (onChange) {
      onChange(images);
    }
  ...
  const thumbs = files?.map((file: any, idx) => {
    ...
    if (file) {
      ...
      const isImage = file?.photoUrl?.thumbnail && imgTypes.includes(fileType); // check if the original filename has the img ext
      // Old Code *******
        ...
        >
          ...
          {!disabled ? (
            <button
              ...
              onClick={() => handleDelete(file?.photoUrl?.thumbnail)}
            >
