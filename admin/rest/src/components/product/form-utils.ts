...
export function getFormattedVariations(variations: any) {
  const variationGroup = groupBy(variations, 'attribute.name');

  return Object.values(variationGroup)?.map((vg) => {
    return {
      attribute: vg?.[0]?.attribute,
      value: vg?.[0]?.value?.map((v) => ({
        id: v._id || v.id,
        value: v?.value,
        meta: v?.meta,
        image: v?.image,
...
export function calculateQuantity(variationOptions: any) {
  return sum(
    variationOptions?.map(({ quantity }: { quantity: string | number }) =>
      Number(quantity),
    ) || [],
  );
}
...
export function getProductInputValues(
  ...
  return {
    ...
    image: omitTypename<any>(image.photoUrl),
    gallery: values.gallery?.map((gi: any) =>
      gi?.photoUrl ? omitTypename(gi.photoUrl) : omitTypename(gi),
    ),
    ...
    ...(product_type?.value === ProductType?.Variable && {
      quantity: calculateQuantity(variationOptions),
      variations: getFormattedVariations(variations),

      variationOptions: {
        // @ts-ignore
        upsert: variationOptions?.map(
          ({
            options,
            id,
            digital_file,
            ...
            ...(image &&
              image.length > 0 && {
                image: image.map((item) => item?.photoUrl),
              }),
            ...(rest?.is_digital && {
              digital_file: {
                id: digital_file?.id,
        ...
        delete: initialValues?.variationOptions
          ?.map((initialVariationOption: Variation) => {
            // @ts-ignore
            const find = variationOptions?.find(
    ...
    ...calculateMinMaxPrice(variationOptions),
  };
}
