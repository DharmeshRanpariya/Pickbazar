...
const OrderTransactionList = ({
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'center',
      width: 120,
      onHeaderCell: () => onHeaderClick('paidTotal'),
      render: function Render(paidTotal: any, record: any) {
    ...
    },
    {
      ...
      key: 'salesTax',
      align: 'center',
      render: function Render(salesTax: any) {
        const sales_tax_amount = salesTax ? salesTax : 0;
        const { price } = usePrice({
          amount: sales_tax_amount,
        });
        return <span>{price}</span>;
    ...
    {
      title: t('table:table-item-flashSaleDiscount'),
      dataIndex: 'flashSaleDiscount',
      key: 'flashSaleDiscount',
      align: 'center',
      render: function Render(flashSaleDiscount: any) {
        const flashSaleDiscount_amount = flashSaleDiscount
          ? flashSaleDiscount
          : 0;
        const { price } = usePrice({
          amount: flashSaleDiscount_amount,
        });
        return <span>{price}</span>;
      },
    },
    {
      ...
      key: 'paymentGateway',
      align: alignLeft,
      render: (paymentGateway: string) => <div>{paymentGateway}</div>,
    },
    {
      ...
      render: (paymentStatus: string) => (
        <Badge text={t(paymentStatus)} color={StatusColor(paymentStatus)} />
      ),
    },
  ];
