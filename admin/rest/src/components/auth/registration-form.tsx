...
const registrationFormSchema = yup.object().shape({
  ...
  permissions: yup.string().default('Admin').oneOf(['Admin']),
});
const RegistrationForm = () => {
  ...
  async function onSubmit({ name, email, password, permissions }: FormValues) {
    registerUser(
      {
        name,
        email,
...

      {
        onSuccess: (data) => {
          if (data?.token) {
            if (hasAccess(admin, data.user.permissions)) {
              setAuthCredentials(data?.token, data.user.permissions);
