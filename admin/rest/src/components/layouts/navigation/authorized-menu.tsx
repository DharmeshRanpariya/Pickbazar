  ...
  return (
    ...
    >
      <Menu.Button className="flex max-w-[150px] items-center gap-2 focus:outline-none lg:py-0.5 xl:py-2.5">
        <Avatar
          src={
            user.profile?.avatar?.thumbnail ?? siteSettings.avatar.placeholder
          }
          rounded="full"
          name="avatar"
          ...
          {isAdmin && (
            <Menu.Item>
              <li className="border-b border-dashed border-gray-200 p-2 focus:outline-none">
                <div
                  className="flex items-center gap-2 rounded-md bg-gray-100 px-3 py-2"
                  key={user.id}
                >
                  <Avatar
                    src={
          ...
          <div className="space-y-0.5 py-2">
            {siteSettings?.authorizedLinks?.map(
              ({ href, labelTransKey, icon, permission }, index) => {
                const hasPermission = permission?.includes('Admin');
                return (
                  <Fragment key={index}>
                    {hasPermission && (
