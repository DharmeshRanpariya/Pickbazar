...
export default function RefundsPage() {
  ...
  const [refundType, setRefundType] = useState(query.status || '');
  ...
  useEffect(() => {
    if (query.status) {
        setRefundType(query.status as string);
    }
}, [query.status]);
    ...
    <>
      <Card className="mb-8 flex flex-col">
        ...
        <div className="mt-5 flex w-full flex-col border-t border-gray-200 pt-5 md:mt-8 md:flex-row md:items-center md:pt-8">
          <CategoryTypeFilter
            className="w-full"
            onRefundTypeFilter={(orderType: RefundTypeOptions) => {
              setRefundType(orderType?.slug);
              setPage(1);
            }}
            enableRefundType
          />
