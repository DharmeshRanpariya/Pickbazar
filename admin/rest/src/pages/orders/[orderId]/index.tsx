...
export default function OrderDetailsPage() {
  ...
  const phoneNumber = useFormatPhoneNumber({
    customer_contact: order?.customerContact as string,
  });
  ...
  const columns = [
    {
      ...
      width: 70,
      render: (product) => {
        const image = product?.image?.[0]?.thumbnail;
        return (
          <div className="relative h-[50px] w-[50px]">
            <Image
              src={image ?? '/placeholder-image-url.jpg'}
    ...
    },
    {
      ...
      align: 'right',
      render: (product, item) => {
        const total = item?.quantity * product?.sale_price;
    ...
    <>
      <Card className="relative overflow-hidden">
        ...
        <div className="flex flex-col items-center lg:flex-row">
          <h3 className="mb-8 w-full whitespace-nowrap text-center text-l font-semibold text-heading lg:mb-0 lg:w-1/3 lg:text-start">
            {t('form:input-label-order-id')} - {order?.trackingNumber}
          </h3>
            ...
            >
              <div className="z-20 w-full me-5">
                <SelectInput
                  ...
                  options={ORDER_STATUS.slice(0, 9)}
        ...
        <div className="my-5 flex items-center justify-center lg:my-10">
          <OrderStatusProgressBox
            ...
            paymentStatus={order?.paymentGateway as PaymentStatus}
          />
        </div>
        {order?.orderStatus === 'order-cancelled' ? (
          <div>
            <span className="">
              {t('form:input-label-orders-cancelReason')} : {order.cancelReason}
            </span>
            <br/>
            <span className="">
              {t('form:input-label-orders-cancelDescription')} :{' '}
            ...
            <>
              <div className="flex w-full flex-col space-y-2 border-t-4 border-double border-border-200 px-4 py-4 ms-auto sm:w-1/2 md:w-1/3">
                ...
                {order?.flashSaleDiscount! > 0 && (
                  <div className="flex items-center justify-between text-sm text-body">
                    <span>{t('text-flashSale-discount')}</span>
                    <span>{flashSaleDiscount}</span>
                  </div>
                )}
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="mb-10 w-full sm:mb-0 sm:w-1/2 sm:pe-8">
            ...
            <div className="flex flex-col items-start space-y-1 text-sm text-body">
              ...
              {order?.customerContact && <span>{phoneNumber}</span>}
            </div>
          </div>
